...
import { PrivacyQuestion, QuestionBox } from "@/components";
import { agreementsData } from "@/constants/general";
...
const AgreementPage = () => {
  ...
  return (
    <div className="mx-[14px] my-24 min-h-[80vh] w-[unset] max-w-[105vh] rounded-[25px] bg-[#0a0d14] p-2.5 px-[14px] text-[small] font-extralight text-white transition-all duration-300 ease-out xs:mx-auto xs:w-full xs:p-[30px] xs:text-[15px]">
      ...
      {agreementsData.map((agreeement) => (
        <QuestionBox
          key={agreeement.id}
          QuestionBoxtitle={agreeement.title}
          QuestionBoxRespond={agreeement.respond}
          index={agreeement.id}
          selected={selected}
